...
def exon_finder(tstart,tend,strand,qstart,qend,qlen,qstartphase,qendphase,seqdict,seqname,
                ...
                cluster = None,exon_number = None,log_file = open(os.devnull, 'w')):
    ...
    if pseudo and args.full_pseudoexon_search and cluster != None and exon_number != None:
...
def recover_missing_exon(strand,cluster,exon_numbers,includes_start,includes_stop, search_coords, log_file):
...
def last_annotation_almost_complete():
    if last_strand == '-':
        ...
        recovered = recover_missing_exon(last_strand,working_name[0].split('coord')[0],range(last_exon_num),is_start,is_stop,[max((last_tend,last_tstart)),min(max((last_tend,last_tstart)) + 2000,min((tstart,tend)))],log_file)
    elif last_strand == '+':
        is_start,is_stop, if_missing = False,True,"C"
        recovered = recover_missing_exon(last_strand,working_name[0].split('coord')[0],range(last_exon_num + 1, last_num_exons),is_start,is_stop,[max((last_tend,last_tstart)),min(max((last_tend,last_tstart)) + 2000,min((tstart,tend)))],log_file)
    if not recovered:
    ...
    for locus in locus_dict.keys():
        ...
        for start_coord_index in range(len(locus_dict[locus].keys()) + 1):
            ...
            if from_start == 0 and (nextfrom_start == 1 or working_annotation == [] or last_from_start > nextfrom_start or last_strand != nextstrand == strand):
                ...
                if strand:
                    if strand == '-':
                        recovered = recover_missing_exon(strand,working_name[0].split('coord')[0],range(exon_num + 1,num_exons + 1),False,True, [max((min((tend,tstart)) - 2000,max((last_tend,last_tstart)))),min((tend,tstart))],log_file)
                    elif strand == '+':
                        recovered = recover_missing_exon(strand,working_name[0].split('coord')[0],range(exon_num),True,False, [max((min((tend,tstart)) - 2000,max((last_tend,last_tstart)))),min((tend,tstart))],log_file)
                else:
                    recovered = False
            ...
            #     working_name = ["oops",0]
                ...
                if from_start > last_from_start + 1:
                    ...
                    recovered = recover_missing_exon(strand,working_name[0].split('coord')[0],range(min([last_exon_num + 1,exon_num + 1]),max([last_exon_num ,exon_num])),False,False,[max((last_tend,last_tstart)),min((tstart,tend))],log_file)
                if from_end != 0:
